...
///All of these are the colors used for the props in the game, and are unchanging throughout said game.
...
///this is how fast the snake moves, increased speed from 0.2 to 0.1 for difficulty
...
pub struct Game {
    ...
    ///new variable meant to track how much food the snake eats per round, resets every game.
    foodcount: i32,

    ///new variable that is meant to track your highest score.
    highscore: i32,
...
impl Game {
    pub fn new(width: i32, height: i32) -> Game {
        Game {
            ///all the properties of the snake itself that is going to be played.
            snake: Snake::new(2, 2),
            waiting_time: 0.0,
            food_exist: true,
            food_x: 5,
    ...
    ///when there is a game over, it resets the game itself.
    ...
    pub fn update(&mut self, delta_time: f64) {
        ...
        if self.is_game_over {
            if self.waiting_time > RESTART_TIME {
                ...
                ///two statements get printed at the end of the game, the score you accumulate for that round and your total high score.
                println!("Game Over! Score: {}", self.foodcount);
                println!("Your highest score is: {}", self.highscore);
                self.restart();
            }
            return;
        }
    ...
    ///this checks to see if the snake has eaten a food pellet at this time, from here I update the high score if the current food count surpasses it.
    fn check_eating(&mut self) {
        ...
        if self.food_exist && self.food_x == head_x && self.food_y == head_y {
            ...
            self.foodcount+=1;
            if self.highscore < self.foodcount {
                self.highscore+=1;
            }
    ...
    ///resets all the snake properties after the game is over.
    fn restart(&mut self) {
